...
- **test: eliminate TOCTOU port-collision flake in integration fixture** — the `_free_port()` helper in `tests/integration/conftest.py` closed its probe socket before the service rebound, leaving a window where the OS could hand the same port to another process. The helper is replaced by `_bound_socket()`, which returns a socket bound to `0.0.0.0:0` (matching the services' actual bind address) and held open. REST/uvicorn services (GCS, BigQuery, Metadata, admin) receive the socket via `uvicorn.Server.serve(sockets=[s])` so the port is held continuously; gRPC services (SecretManager, PubSub, Firestore) close the socket in the same call-frame just before `grpc.aio.Server.start()`, reducing the hand-off window to microseconds. `Context` gains an optional `sockets` field; `cli.run()` gains an optional `sockets` keyword argument. ([#26](https://github.com/aGallea/gcp-local/issues/26))
